...
class HiCDataset(Dataset):
    ...
    def __init__(self, metadata, data_res, resolution, stride=8, exclude_chroms=['chrY', 'chrX', 'Y', 'X', 'chrM', 'M'],
                 reference='mm9'):
        """
        Args:
        ...
        """
        self.reference, self.data_res, self.resolution, self.split_res, self.pixel_size = reference, data_res, resolution, int(
            resolution / stride), int(resolution / data_res)
        self.metadata = {'filename': metadata[0], 'replicate': metadata[1], 'norm': metadata[2],
                         'type_of_bin': metadata[3], 'class_id': metadata[4], 'chromosomes': OrderedDict()}
        self.positions = []
    ...
    def get_genomic_positions(self, append=''):
        ...
        return {"Chromosome": chromosomes, "Start": np.array(self.positions),
                "End": np.array(self.positions) + self.resolution}
...
class TripletHiCDataset(HiCDataset):
    ...
    def __init__(self, list_of_HiCDatasets, reference=reference_genomes["mm9"]):
        ...
        if not checks:
            return None
        self.resolution, self.data_res, self.split_res = list_of_HiCDatasets[0].resolution, list_of_HiCDatasets[
            0].data_res, list_of_HiCDatasets[0].split_res
    ...
    def check_input(self, list_of_HiCDatasets):
        """Check if input datasets are valid."""
        filenames_norm = set()
        if not isinstance(list_of_HiCDatasets, list):
            print("list of HiCdatasets is not list type")
            return False
    ...
    def append_data(self, curr_data, pos, mode='exhaustive'):
        ...
        優化後的數據生成函數
        mode: 
        - 'exhaustive': 全面對齊，每個樣本都當過 Anchor (每個 window 生成 8 組)
        - 'paired': 配對對齊，固定正樣本對 (每個 window 生成 4 組)
        """
        if len(curr_data) < 2:
            return
        ...
        # 2. 生成 Triplets
        for anchor_class, samples in class_groups.items():
            if len(samples) < 2:
                continue

            for i, anchor in enumerate(samples):
                # 根據 mode 決定 Positive 的選取範圍
                if mode == 'exhaustive':
                    # 全面遍歷：除了自己以外的都是 Positive
                    pos_indices = [idx for idx in range(len(samples)) if idx != i]
                else: # mode == 'paired'
                    # 簡化遍歷：只往後找，減少重複配對
                    pos_indices = range(i + 1, len(samples))

                for j in pos_indices:
                    positive = samples[j]
                    
                    # 遍歷不同類別的 Negative
                    for other_class, other_samples in class_groups.items():
                        if other_class == anchor_class:
                            continue
                        
                        for negative in other_samples:
                            self.data.append((
                                anchor[0],    # anchor image/path
                                positive[0],  # positive image/path
                                negative[0],  # negative image/path
                                anchor_class  
                            ))
...
class PairOfDatasets(TripletHiCDataset):
    from scipy import ndimage
    """Triplet Hi-C datasets by genomic location to create feature map for triplets."""
    ...
    def make_maps_base(self, chromosome, diagonal_off=4):
        """Create base feature maps for triplets."""
        ...
        # Initialize maps for each triplet combination
        for map_combo in [(j, i, k) for j in range(len(self.metadata))
                          for i in range(j + 1, len(self.metadata))
                          for k in range(len(self.metadata)) if k != j and k != i]:
            triplet_maps[map_combo] = {}
            triplet_maps[map_combo]["rotated_shapes"] = np.zeros(dims)
            triplet_maps[map_combo]["norm"] = np.zeros(dims)

        for ind in range(chrom_index2, chrom_index1, -1):
            true_ind = ind - 1
            for curr_filt in range(nfilter):
                x = self.data[true_ind][curr_filt]
                x = (x + np.transpose(x)) / 2
                x = np.multiply(x, (
                            np.tril(np.ones(x.shape[0]), -diagonal_off) + np.triu(np.ones(x.shape[1]), k=diagonal_off)))
                rotated = self.ndimage.rotate(x, angle=45, reshape=True)
                rotated = resize(rotated, dataset_dims)

                map_key = self.labels[true_ind]
                triplet_maps[map_key]["rotated_shapes"][curr_filt, :,
                int(self.positions[true_ind] / self.data_res):int(
                    (self.positions[true_ind] + self.resolution) / self.data_res)] += rotated
                triplet_maps[map_key]["norm"][curr_filt, :,
                int(self.positions[true_ind] / self.data_res):int(
                    (self.positions[true_ind] + self.resolution) / self.data_res)] += 1

        # Normalize maps
        all_maps = {triplet: triplet_maps[triplet]["rotated_shapes"] / triplet_maps[triplet]["norm"]
                    for triplet in triplet_maps.keys()}
        return all_maps

    def make_maps_grouped(self, all_maps):
        """Group feature maps by triplet relationships."""
        if all_maps is None: return None
        all_maps_grouped = {}
        maps_metadata = {i: metadata["class_id"] for i, metadata in enumerate(self.metadata)}

        all_maps_grouped = {"anchor_positive": np.zeros_like(list(all_maps.values())[0]),
                            "anchor_negative": np.zeros_like(list(all_maps.values())[0])}
        ap_count, an_count = 0, 0

        for triplet, feature_map in all_maps.items():
            if np.all(np.isnan(feature_map)): continue

            # Group by relationship type
            if maps_metadata[triplet[0]] == maps_metadata[triplet[1]]:  # anchor-positive
                all_maps_grouped["anchor_positive"] += feature_map
                ap_count += 1
            else:  # anchor-negative
                all_maps_grouped["anchor_negative"] += feature_map
                an_count += 1

        # Normalize grouped maps
        if ap_count > 0:
            all_maps_grouped["anchor_positive"] /= ap_count
        if an_count > 0:
            all_maps_grouped["anchor_negative"] /= an_count

        return all_maps_grouped

    def extract_features(self, chromosome, nfilter, triplet_type, qthresh=0.999,
                         min_length=10, max_length=256, min_width=10, max_width=256,
                         pad_extra=3, im_size=20):
        """Extract features from triplet maps."""
        # Similar to PairOfDatasets.extract_features but adapted for triplet structure
        if nfilter == "all":
            curr_map = np.concatenate([
                np.sum(value["anchor_positive"][:, :int(self.pixel_size / 2), :], axis=0)
                for chrom, value in self.paired_maps.items() if value is not None
            ], axis=1)
        else:
            curr_map = np.concatenate([
                value["anchor_positive"][nfilter, :int(self.pixel_size / 2), :]
                for chrom, value in self.paired_maps.items() if value is not None
            ], axis=1)
        ...
        if nfilter == "all":
            curr_map = np.sum(self.paired_maps[chromosome][triplet_type][:, int(self.pixel_size / 2):, :], axis=0)
        else:
            curr_map = self.paired_maps[chromosome][triplet_type][nfilter, int(self.pixel_size / 2):, :]

        # Extract features using the same logic as PairOfDatasets
        ...
        for pos_or_neg, arr in enumerate([arr_pos, arr_neg]):
            for feature_index in np.unique(arr[0])[1:]:
                ...
                if ((x2 - x1) <= min_length) or ((y2 - y1) <= min_width): continue
                if ((x2 - x1) >= max_length) or ((y2 - y1) >= max_width): continue

                temp = convex_hull_image(arr[0] == feature_index)
                temp = temp[x1:x2, y1:y2]

                if (temp.shape[0] <= min_length) or (temp.shape[1] <= min_width): continue
                if (temp.shape[0] >= max_length) or (temp.shape[1] >= max_width): continue
                ...
                features.append((feature_index, temp, original_dims, height, arr[1],
                                 pos_or_neg, qthresh, [chromosome, x1, x2, y1, y2]))
        return features

    def make_maps(self, chromosome, diagonal_off=4):
        """Create and group feature maps for chromosome."""
...
#
#     def append_data(self, curr_data, pos):
#         self.data.extend([(self.model.features(curr_data[k][0].unsqueeze(0)).detach().numpy() - self.model.features(
#             curr_data[j][0].unsqueeze(0)).detach().numpy())[0] for k in range(0, len(curr_data)) for j in
#                           range(k + 1, len(curr_data))])
#         self.positions.extend([pos for k in range(0, len(curr_data)) for j in range(k + 1, len(curr_data))])
#         self.labels.extend([(k, j) for k in range(0, len(curr_data)) for j in range(k + 1, len(curr_data))])
#
#     def make_maps_base(self, chromosome, diagonal_off=4):
...
#                 rotated = resize(rotated, dataset_dims)
#                 pair_maps[self.labels[true_ind]]["rotated_shapes"][curr_filt, :,
#                 int(self.positions[true_ind] / self.data_res):int(
#                     (self.positions[true_ind] + self.resolution) / self.data_res)] += rotated
#                 pair_maps[self.labels[true_ind]]["norm"][curr_filt, :,
#                 int(self.positions[true_ind] / self.data_res):int(
#                     (self.positions[true_ind] + self.resolution) / self.data_res)] += 1
#
...
#                 rep_pairs += 1
#                 if "replicate" in all_maps_grouped:
#                     all_maps_grouped["replicate"] += all_maps[pairs]
#                 else:
#                     all_maps_grouped["replicate"] = all_maps[pairs]
#             else:
#                 cond_pairs += 1
#                 if "conditions" in all_maps_grouped:
#                     all_maps_grouped["conditions"] += all_maps[pairs]
#                 else:
#                     all_maps_grouped["conditions"] = all_maps[pairs]
#
...
#         return all_maps_grouped
#
#     def extract_features(self, chromosome, nfilter, pair, qthresh=0.999, min_length=10, max_length=256, min_width=10,
#                          max_width=256, pad_extra=3, im_size=20):
#         if nfilter == "all":
#             curr_map = np.concatenate(
#                 [np.sum(value["replicate"][:, :int(self.pixel_size / 2), :], axis=0) for chrom, value in
#                  self.paired_maps.items()
#                  if value is not None], axis=1)
#         else:
#             curr_map = np.concatenate(
#                 [value["replicate"][nfilter, :int(self.pixel_size / 2), :] for chrom, value in self.paired_maps.items()
#                  if value is not None], axis=1)
#
#         pos_thresh = np.max([np.nanquantile(curr_map, qthresh), -np.nanquantile(curr_map, 1 - qthresh)])
#         neg_thresh = np.min([-np.nanquantile(curr_map, qthresh), np.nanquantile(curr_map, 1 - qthresh)])
#
#         if nfilter == "all":
#             curr_map = np.sum(self.paired_maps[chromosome][pair][:, int(self.pixel_size / 2):, :], axis=0)
#         else:
#             curr_map = self.paired_maps[chromosome][pair][nfilter, int(self.pixel_size / 2):, :]
#
#         arr_pos, arr_neg = (curr_map > pos_thresh), (curr_map < neg_thresh)
#         arr_pos, arr_neg = ndimage.label(arr_pos), ndimage.label(arr_neg)
#
...
#                 original_dims, height = temp.shape, np.min(indices[0])
#                 temp = resize(temp, (im_size, im_size), anti_aliasing=False, preserve_range=True)
#                 features.append((feature_index, temp, original_dims, height, arr[1], pos_or_neg, qthresh,
#                                  [chromosome, x1, x2, y1, y2])
#         return features
#
#     def make_maps(self, chromosome, diagonal_off=4):
#         all_maps = self.make_maps_base(chromosome, diagonal_off=diagonal_off)
