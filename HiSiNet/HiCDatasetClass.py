...
class HiCDatasetDec(HiCDataset):
   ...
   def add_chromosome(self, chromosome):
       if (chromosome in self.metadata['chromosomes'].keys()) or (
               chromosome[3:] in self.metadata['chromosomes'].keys()):
           return print('chromosome already loaded')
   ...
   def get_chromosome(self, straw_file, chromosome):
       ...
       for start_pos in range(first, last, self.split_res):
           self.make_matrix(straw_matrix, start_pos, start_pos + self.resolution - self.data_res, chromosome)
   ...
   def make_matrix(self, straw_matrix, start_pos, end_pos, chromosome):
       ...
       if (len(set(xpos)) < self.pixel_size * 0.9) or (np.sum(np.isnan(vals)) > 0.5 * len(vals)):
           return None
...
class GroupedHiCDataset(HiCDataset):
   def __init__(self, list_of_HiCDatasets):
       ...
       if not isinstance(list_of_HiCDatasets, list):
           print("list of HiCDataset is not list type")
       self.resolution, self.data_res = list_of_HiCDatasets[0].resolution, list_of_HiCDatasets[0].data_res
       for dataset in list_of_HiCDatasets:
           self.add_data(dataset)

   def add_data(self, dataset):
       if not isinstance(dataset, HiCDataset):
           return print("file not HiCDataset")
...
class SiameseHiCDataset(HiCDataset):
   ...
   def check_input_parameters(self, dataset):
       pass
   ...
   def append_data(self, curr_data, pos):
       self.data.extend(
           [(curr_data[k][0], curr_data[j][0], (self.sims[0] if curr_data[k][1] == curr_data[j][1] else self.sims[1]))
            for k in range(len(curr_data)) for j in range(k + 1, len(curr_data))])
       self.positions.extend([pos for k in range(len(curr_data)) for j in range(k + 1, len(curr_data))])
       self.labels.extend([(k, j) for k in range(len(curr_data)) for j in range(k + 1, len(curr_data))])
...
class HiCDatasetCool(HiCDataset):
   ...
   def get_chromosome(self, cl_file, chromosome):
       ...
       initial = len(self.positions)
       self.metadata['chromosomes'][chromosome] = []
       for start_pos in range(first, last, stride):
           self.make_matrix(cl_matrix, start_pos, first)
       self.metadata['chromosomes'][chromosome] = (initial, len(self.positions))
   ...
   def make_matrix(self, cl_matrix, start_pos, first):
       ...
       if sum(np.diagonal(np.isnan(image_scp) | (image_scp == 0))) > self.pixel_size * 0.9:
           return None
       image_scp[np.isnan(image_scp)] = 0
...
class PairOfDatasets(SiameseHiCDataset):
   ...
   def append_data(self, curr_data, pos):
       self.data.extend([(self.model.features(curr_data[k][0].unsqueeze(0)).detach().numpy() - self.model.features(
           curr_data[j][0].unsqueeze(0)).detach().numpy())[0] for k in range(len(curr_data)) for j in
   ...
   def make_maps_base(self, chromosome, diagonal_off=4):
       ...
       for (map1, map2) in [(j, i) for j in range(len(self.metadata)) for i in range(j + 1, len(self.metadata))]:
           pair_maps[(map1, map2)] = {}
