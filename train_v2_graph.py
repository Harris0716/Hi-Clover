...
# make sure HiCDatasetClass returns 4 values
...
# 2. distance evolution (line graph)
...
# target 5000 points, average allocation to each raw file
samples_per_file = max(1, 5000 // len(val_paths))

with torch.no_grad():
    for p in val_paths:
        # directly sample from mlhic to avoid triplet pairing bias
        ...
        for batch in temp_loader:
            ...
            class_ids = batch[-1].cpu().numpy() # original class_id (1=NIPBL, 2=TAM)
...
# convert to Numpy and ensure length alignment
...
# 3. plot (4 color scheme)
# deep blue (NIPBL R1), light blue (NIPBL R2), deep red (TAM R1), light red (TAM R2)
