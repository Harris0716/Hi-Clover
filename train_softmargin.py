...
# Argument Parser
# ---------------------------------------------------------
parser = argparse.ArgumentParser(description='Triplet network (v6)')
...
try:
    for epoch in range(args.epoch_training):
        ...
        for i, data in enumerate(train_loader):
            ...
            # ── Semi-Hard Mining (batch-internal) ─────────────────
            # 注意：semi-hard 的視窗 d(a,p) < d(a,n) < d(a,p) + margin 需要 margin。
            # soft_margin 模式下 loss 沒有 margin，但 semi-hard 仍沿用 args.margin 當視窗寬度。
            if args.semi_hard:
                B = a.size(0)
                sh_window = args.margin  # semi-hard 視窗寬度（即使 soft_margin 也用此值篩選）
                with torch.no_grad():
                    ...
                    # Semi-hard 視窗:  d(a,p) < d(a,n) < d(a,p) + sh_window
                    sh_cond = (dist_mat > d_ap.unsqueeze(1)) & \
                              (dist_mat < (d_ap + sh_window).unsqueeze(1))
                    sh_mask = diff_class & sh_cond                            # [B, 3B]

                    large = torch.tensor(float('inf'), device=device)

                    # 1) Semi-hard 候選中距離最小者
                    sh_dist = torch.where(sh_mask, dist_mat, large)
                    sh_min_d, sh_min_idx = sh_dist.min(dim=1)
                    has_sh = torch.isfinite(sh_min_d)                        # [B] bool

                    # 2) Fallback: 沒有 semi-hard 時，取 hardest negative（異 class 中最近者）
                    hard_dist = torch.where(diff_class, dist_mat, large)
                    _, hard_min_idx = hard_dist.min(dim=1)

                    selected_idx = torch.where(has_sh, sh_min_idx, hard_min_idx)  # [B]
