...
parser.add_argument('--joint_loss', action='store_true', help='Use joint triplet + cross-entropy loss')
parser.add_argument('--ce_weight', type=float, default=0.5, help='Weight for cross-entropy loss in joint loss (lambda)')
...
ce_criterion = None
if args.joint_loss:
    cls_head = nn.Linear(args.embedding_dim, 2).to(device)
    nn.init.xavier_uniform_(cls_head.weight)
    nn.init.constant_(cls_head.bias, 0)
...
try:
    for epoch in range(args.epoch_training):
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.joint_loss:
                batch_size = a.size(0)
                ce_logits = torch.cat([cls_head(a_out), cls_head(p_out), cls_head(n_out)], dim=0)
                ce_labels = torch.cat([
                    torch.zeros(batch_size, dtype=torch.long, device=device),
                    torch.zeros(batch_size, dtype=torch.long, device=device),
                    torch.ones(batch_size, dtype=torch.long, device=device)
                ])
                ce_loss = ce_criterion(ce_logits, ce_labels)
                loss = triplet_loss + args.ce_weight * ce_loss
            else:
                loss = triplet_loss
