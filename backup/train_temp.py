...
for epoch in range(args.epoch_training):
    ...
    for i, data in enumerate(train_loader):
        ...
        idx = torch.randperm(ao.size(0), device=device)
        ...
        if (i + 1) % 100 == 0 or (i + 1) == len(train_loader):
            with torch.no_grad():
                ...
                das = F.pairwise_distance(ao, spatial_negative).mean().item()
            print(f"Epoch [{epoch+1}/{args.epoch_training}], Step [{i+1}/{len(train_loader)}], Loss: {running_loss/(i+1):.4f}, d(a,p): {dap:.4f}, d(cond): {dan:.4f}, d(spatial): {das:.4f}")
