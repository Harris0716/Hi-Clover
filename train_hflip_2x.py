...
# [CHANGED] h_flip moved from training loop to dataset level (2x data)
import numpy as np
import torch
...
parser.add_argument('--anti_diag_flip', action='store_true', help='Anti-Diagonal Flip (random 50% per batch)')
parser.add_argument('--h_flip', action='store_true', help='Horizontal Flip (2x dataset augmentation)')
...
# [CHANGED] h_flip is now applied at the dataset level (2x data), not in the training loop
...
hflip_str = " | HFlip: 2x dataset" if args.h_flip else ""
sched_label = {"plateau": "ReduceLROnPlateau", "cosine": "CosineAnnealingLR", "none": "None"}[args.scheduler]
print(f"Starting training: {file_param_info} | Optimizer: {args.optimizer.upper()} | Scheduler: {sched_label}" + wd_str + semi_str + hflip_str)
...
try:
    for epoch in range(args.epoch_training):
        ...
        for i, data in enumerate(train_loader):
            ...
            # Anti-diagonal flip (still random 50% per batch, if enabled)
            ...
            # [CHANGED] h_flip is no longer done here — it's in the dataset (2x)
...
if args.h_flip: aug_features.append("HFlip-2x")
