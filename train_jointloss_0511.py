...
# [CHANGED] Joint loss initialization: pair-wise BCE
scheduler = None
ce_criterion = None
pair_clf = None
if args.joint_loss:
    ce_criterion = nn.BCEWithLogitsLoss()
    # pair_classifier is already inside the model (TripletLeNetBatchNormSE_Joint)
    # its parameters are included in model.parameters(), no need for add_param_group
...
joint_str = f" | Joint BCE (λ={args.ce_weight})" if args.joint_loss else ""
