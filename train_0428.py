...
# [Modified] Added Joint Loss (Pairwise Classification) Support
...
# [New] Joint Loss Weight
parser.add_argument('--lambda_ce', type=float, default=0.5, help='Weight for pairwise classification BCE loss')
...
# parameters
# ---------------------------------------------------------
file_param_info = f"{args.model_name}_{args.optimizer}_{args.scheduler}_{args.learning_rate}_{args.batch_size}_{args.seed}_{args.margin}_CE{args.lambda_ce}"
...
# [New] Binary Cross Entropy Loss for Joint Loss
...
# Helper function to handle DataParallel for Joint Loss
def get_pair_logits(net, emb1, emb2):
    if isinstance(net, nn.DataParallel):
        return net.module.forward_pair_classify(emb1, emb2)
    return net.forward_pair_classify(emb1, emb2)
...
try:
    for epoch in range(args.epoch_training):
        ...
        for i, data in enumerate(train_loader):
            ...
            # Select which embeddings to use for loss calculation
            if args.semi_hard:
                with torch.no_grad():
                ...
                if sh_mask.any():
                    ...
                    a_use, p_use, n_use = a_out[sh_mask], p_out[sh_mask], n_out[sh_mask]
                else:
                    fallback_count += 1
                    a_use, p_use, n_use = a_out, p_out, n_out
            else:
                a_use, p_use, n_use = a_out, p_out, n_out

            # --- [New] Calculate Losses ---
            # 1. Triplet Loss
            loss_triplet = criterion(a_use, p_use, n_use)

            # 2. Pairwise Classification Loss (Joint Loss)
            # Predict (A, P) pairs as 1 (replicates / same condition)
            logits_ap = get_pair_logits(model, a_use, p_use)
            labels_ap = torch.ones_like(logits_ap)
            loss_ap = bce_criterion(logits_ap, labels_ap)

            # Predict (A, N) pairs as 0 (conditions / different conditions)
            logits_an = get_pair_logits(model, a_use, n_use)
            labels_an = torch.zeros_like(logits_an)
            loss_an = bce_criterion(logits_an, labels_an)

            # Total Loss
            loss = loss_triplet + args.lambda_ce * (loss_ap + loss_an)
            # -------------------------------
            
            # Normalize and backpropagate
            ...
            if (i + 1) % 100 == 0 or (i + 1) == len(train_loader):
                ...
                print(f"Epoch [{epoch+1}/{args.epoch_training}], Step [{i+1}/{len(train_loader)}], "
                      f"Loss: {running_loss/(i+1):.4f} (Trip: {loss_triplet.item():.4f}, BCE: {(loss_ap+loss_an).item():.4f}), "
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                # --- [New] Validation Joint Loss ---
                loss_triplet_val = criterion(ao, po, no)
                logits_ap_val = get_pair_logits(model, ao, po)
                loss_ap_val = bce_criterion(logits_ap_val, torch.ones_like(logits_ap_val))
                logits_an_val = get_pair_logits(model, ao, no)
                loss_an_val = bce_criterion(logits_an_val, torch.zeros_like(logits_an_val))
                
                total_val_loss = loss_triplet_val + args.lambda_ce * (loss_ap_val + loss_an_val)
...
aug_features.append(f"CE_{args.lambda_ce}") # Show CE weight in title
...
ax[0].plot(train_losses, label='Train (Trip+CE)')
ax[0].plot(val_losses, label='Val (Trip+CE)')
ax[0].set_title('Total Loss Evolution')
ax[0].legend()
