...
best_epoch       = -1  # 0-based epoch index of the best validation triplet loss
patience_counter = 0
...
try:
    for epoch in range(args.epoch_training):
        ...
        if avg_triplet < best_val_loss:
            ...
            best_epoch       = epoch  # 0-based; display as epoch + 1
            patience_counter = 0
...
# ---------------------------------------------------------
# Save training history for reproducible plotting
# ---------------------------------------------------------
# This .npz stores all epoch-level values needed to redraw the training_stats figure.
# val_losses is the early-stopping metric; best_epoch is 0-based and displayed as best_epoch + 1.
history_path = base_save_path + "_history.npz"
completed_epochs = len(val_losses)
epoch_numbers = np.arange(1, completed_epochs + 1)
...
np.savez_compressed(
    ...
    val_bce_losses=np.array(val_bce_losses, dtype=float),
    val_log_ratio_history=np.array(val_log_ratio_history, dtype=float),
    grad_norm_backbone_history=np.array(grad_norm_backbone_history, dtype=float),
    grad_norm_clf_history=np.array(grad_norm_clf_history, dtype=float),
    lr_history=np.array(lr_history, dtype=float),
    best_epoch=np.array(best_epoch, dtype=int),
    best_val_loss=np.array(best_val_loss, dtype=float),
)
print(f"Training history saved: {history_path}")
if best_epoch >= 0:
    print(f"Best epoch: {best_epoch + 1}, Best Val Triplet: {best_val_loss:.4f}")
...
show_lr_plot = len(lr_history) > 0
n_cols = 5 if (args.joint_loss and show_lr_plot) else \
         4 if (args.joint_loss or show_lr_plot) else 3

fig1, axes = plt.subplots(1, n_cols, figsize=(6 * n_cols, 6))
col = 0

plot_epochs = np.arange(1, len(val_losses) + 1)

axes[col].plot(plot_epochs, train_losses, label='Train Triplet')
axes[col].plot(plot_epochs, val_losses,   label='Val Triplet')
if best_epoch >= 0 and len(val_losses) > best_epoch:
    ...
    axes[col].set_title(
        f'Triplet Loss (early-stopping metric)\n'
        f'Best Val={best_val_loss:.4f} @ Epoch {best_epoch_display}'
    )
else:
...
if args.joint_loss:
    axes[col].plot(plot_epochs, val_bce_losses, color='purple', label='Val BCE')
    axes[col].set_title('Val BCE Loss (diagnostic)')
    axes[col].set_xlabel('Epoch')
    axes[col].legend()
    col += 1

axes[col].plot(plot_epochs, val_log_ratio_history, color='blue')
axes[col].set_title('Log-Ratio log(d(a,n)/d(a,p))')
axes[col].set_xlabel('Epoch')
axes[col].axhline(0, color='k', ls='--')
col += 1

axes[col].plot(plot_epochs, grad_norm_backbone_history, color='teal',   label='Backbone')
if args.joint_loss:
    axes[col].plot(plot_epochs, grad_norm_clf_history,  color='orange', label='Classifier')
...
if show_lr_plot:
    axes[col].plot(plot_epochs, lr_history, color='orange')
    axes[col].set_title('Learning Rate')
    axes[col].set_xlabel('Epoch')
    if np.all(np.array(lr_history) > 0):
